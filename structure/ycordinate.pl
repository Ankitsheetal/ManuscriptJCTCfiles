		...
		if ($line ne '') {
				...
				if (length($line) >= 6) {
					...
					if (($line_id eq 'ATOM  ') or ($line_id eq 'HETATM') or ($line_id eq 'TER   ')) {
						if (($input->{'from'} ne '') or ($input->{'to'} ne '')) {
							my $res_num = trim(substr($line,39,7));
							if ($input->{'from'} ne '') {
			...
			if ($change_this_line == 1) {
				if (length($line) >= 6) {
					my $line_id = substr($line,0,6);
					if (($line_id eq 'ATOM  ') or ($line_id eq 'HETATM') or ($line_id eq 'TER   ')) {
						my $bit1 = substr($line,0,39);
						my $bit2 = substr($line,46);
						my $old_res_num = trim(substr($line,39,7));
						my $new_res_num = sprintf("%.3f", $old_res_num + $add);
						my $new_cord = sprintf("%.3f", $new_res_num);
						my $display_res_num = sprintf("%7s", $new_cord);
